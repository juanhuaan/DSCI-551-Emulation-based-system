...
class CommandView(APIView):
    ...
    def post(self, request):
        ...
        if (route == "mkdir_or_put"):
            ...
            newPath = Paths.objects.create(
                inode=newId, pathType=type, name=newDir)
            newPath.save()
            newRelation = Relations.objects.create(
                parent=upperObj, child=newPath)
            newRelation.save()
            if (type == "FILE"):
    ...
    def put(self, request):
        ...
        newPath = Paths.objects.create(
            inode=rootId, pathType="DIRECTORY", name=rootName)
        newPath.save()
        paths = Paths.objects.all()
        serializer = PathSpecsSerializer(paths, many=True)
        return Response(serializer.data)
    ...
    def delete(self, request):
        ...
        if (route == 'deleteAllPath'):
            ...
            cur_inode = self.get_current_path_inode(
                request.data['absolute_path'])
